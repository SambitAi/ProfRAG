...
def _load_doc_images(document_folder: Path) -> list[dict[str, Any]]:
    """Return image metadata for all document images (for LLM prompt context only)."""
...
def run(
    ...
    try:
        ...
        # Old records lack item_type — fall back to unfiltered query
        logger.warning(
    ...
    try:
        ...
        pass  # media retrieval is best-effort
    ...
    if expand_parent:
        ...
        # Collect abs_paths already in direct chunk results to avoid duplication
